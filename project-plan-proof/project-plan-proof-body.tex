...
Potential high risk reach goals are: web editor, dependent type support, and performance benchmark. First, we can implement a web editor that helps with debugging code with a GUI component. Second, we can develop a fully dependent type support for boundary checks. Finally, we can compare the performance of the compiled version versus the generated original code.
