  ...
  (local [(define (helper stx)
            (match (syntax->list stx)
              ...
              [(list (? (stx-atom? 'typed_identifier))
                     ...
                     (? syntax? f-stx)
                     (? (stx-atom? "]")))
               (local [(define col_type (stx->t t-stx))]
                 (cond [(symbol=? 'list col_type)
                        (i-id-type (stx->id id-stx) (matrixT (stx->t f-stx) 1 UNBOUNDED))]
                       [(symbol=? 'matrix col_type)
                        (i-id-type (stx->id id-stx) (matrixT (stx->t f-stx) UNBOUNDED UNBOUNDED))]))]
              [(list (? (stx-atom? 'typed_identifier))
                     (? syntax? id-stx)
                     (? (stx-atom? ":"))
                     (? syntax? t-stx)
...
(define (test6) (open-input-string (file->string "examples/test_matrices_and_lists.m")))
(define test6-tokens (token-list (tokenize (test6))))
(define test6-parsed (parse (tokenize (test6))))
(define test6-intermediate (parse-intermediate test6-parsed))
(define test6-ast (desugar test6-intermediate))
